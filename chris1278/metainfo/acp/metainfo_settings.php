...
class metainfo_settings
{
	...
	public function main()
	{
		...
		if ($this->request->is_set_post('submit'))
		{
			...
			$this->config->set('cmi_mt_change', $this->request->variable('cmi_mt_change', 0));
			$this->config->set('cmi_meta_title', $this->request->variable('cmi_meta_title', '', true));
		...
		$this->template->assign_vars([
			'CMI_METAINFO_EXT_NAME'							=>	$ext_display_name,
			'CMI_METAINFO_EXT_VER'							=>	$ext_display_ver,
			'CMI_SITENAME'									=>	$this->config['sitename'],
			'CMI_MT_CHANGE'									=>	$this->config['cmi_mt_change'],
			'CMI_META_TITLE'								=>	$this->config['cmi_meta_title'],
